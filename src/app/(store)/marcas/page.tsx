...
// Marcas estáticas como fallback (serão sobrescritas pelas do banco)
...
export default async function MarcasPage() {
    ...
    const listaMarcas = (marcas && marcas.length > 0) ? marcas : MARCAS_FALLBACK
            ...
            <main className="min-h-screen bg-black">
                ...
                <div className="border-b border-white/5 py-16 px-4 sm:px-6 lg:px-8 max-w-[1400px] mx-auto">
                    ...
                    <p className="text-neutral-500 mt-4 font-bold tracking-widest uppercase text-xs">
                        {listaMarcas.length} MARCAS DO ACERVO
                    </p>
                </div>
                ...
                <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-12">
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                        {listaMarcas.map((marca: { name: string; slug: string }) => (
                            <Link
                                key={marca.slug}
